...
│     └─ utils.py                     # shaping-функции + эпизодная декомпозиция награды
...
└─ MARL Reward Shaping/               # исторический snapshot старой дипломной сдачи
...
- `ring_function(d, center, half_width)` - знаковый "бублик": положителен строго внутри sweet-spot, равен нулю на границах и отрицателен вне зоны.
- `compute_ring_bonus_from_state(...)` - state-based бонус по текущему состоянию.
- `RewardShapingEpisodeMetrics` - накопитель полной эпизодной декомпозиции награды в метрики `rc/*`.
...
- **SB**: считается внутри `reward_battle()` как средний знаковый `ring_function(d_min)` по живым союзникам. Позиция внутри диапазона 3–6 получает положительный сигнал, вне диапазона — отрицательный.
...
Для финального протокола параметры заморожены до просмотра результатов:

- `rc_weight=1`: заранее выбранный единичный масштаб, а не результат подбора по test win rate. Работа не заявляет, что это оптимальное значение.
- `max_shaping_abs=1.0`: фиксированный абсолютный кап итоговой добавки за transition. Единичный cap соответствует естественному диапазону нормированных AB/SB-сигналов, поэтому не стирает их градацию; в комбинациях он ограничивает сумму компонентов. Кап не зависит от случайной величины sparse base reward.
...
Чистый PBRS по умолчанию не клиппируется. Старое имя аргумента `max_shaping_ratio` временно принимается Python-классами только для совместимости старых конфигов; финальные YAML используют `max_shaping_abs`.
...
- shaping-окружения отдельно добавляют полную декомпозицию эпизода через `RewardShapingEpisodeMetrics`: `rc/base_return`, `rc/action_return`, `rc/state_return`, `rc/potential_return`, `rc/total_raw_return`, `rc/total_applied_return`, значения на шаг, долю отрицательных/клиппированных шагов и поведенческие diagnostics.

`shaping/*` описывает поведение и состояние базовой SC2-среды; `rc/*` описывает только механизм reward shaping. Компоненты `rc/*` хранятся в исходном, нескейленном reward-space, но terminal win/defeat reward учитывается в `rc/base_return`.

Исторические `final_tb_logs` были собраны до этой схемы: часть shaping-метрик там отражала последний transition, а не эпизод. Их можно показывать только как exploratory-результаты; смешивать их с финальным прогоном или делать по ним количественные выводы нельзя.
...
Скрипт автоматически строит отдельные кривые для всех TensorBoard scalar tags,
включая 18 компонентных `rc/*` метрик; имена с `/` сохраняются в PNG как `__`.
Пакет `tensorboard` для чтения event-файлов устанавливается через
`install_dependencies.sh`/`bootstrap.sh`.
...
- включает commit SHA и параметры протокола в manifest identity, поэтому resume
  не смешивает результаты разных версий кода или разных бюджетов,
- отказывается стартовать из dirty tracked-worktree,
...
Продолжение после сбоя выполняется той же командой на том же commit. Для
намеренного полного перезапуска использовать `RESUME=0` или другой
`RESULTS_ROOT`. `ALLOW_DIRTY_RUN=1` допустим только для одноразового пилота:
...
Каталог `MARL Reward Shaping/` — замороженный исторический snapshot старой
дипломной сдачи, включая старые one-seed логи и прежнюю реализацию shaping. Он
не участвует в импортах/запуске и не является source of truth. Исполняемая и
актуальная версия проекта находится только в корне репозитория; перед новой
публикацией пакет нужно регенерировать из зафиксированного финального commit.
