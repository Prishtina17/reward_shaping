# envs/starcraft/melee_range_control_asp.py
...
class Starcraft2EnvRewardShaping(StarCraft2Env):
    """Action + State + Potential shaping combined.
    """
    ...
    def reward_battle(self) -> float:
        ...
        # Cap only the total sum
        cap = self._max_ratio * max(1.0, abs(float(base)))
        total = rc_bonus_act_raw + rc_bonus_state_raw + shaped_delta_raw
        total = float(np.clip(total, -cap, +cap))

        shaped = float(base) + total
