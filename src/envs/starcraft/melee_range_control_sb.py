...
class Starcraft2EnvRewardShaping(StarCraft2Env):
    ...
    def _compute_rc_bonus(self) -> None:
        rc_raw_sum = 0.0
        ...
        if n_alive == 0:
            self._pending_action_bonus = 0.0
            total_allies = max(1, self.n_agents)
            total_enemies = max(1, self.n_enemies)
            alive_allies = count_alive_allies(self)
            alive_enemies = count_alive_enemies(self)
            cache["ally_alive"] = float(alive_allies)
            cache["enemy_alive"] = float(alive_enemies)
        ...
        ring_raw_mean *= float(self._rc_weight)



        self._pending_action_bonus = float(ring_raw_mean)
        ...
        # эпизодные агрегаты
        self._ep_rc_ring_time_sum += float(rc_in_ring_count / n_alive)
        self._ep_rc_oor_time_sum += float(rc_oor_count / n_alive)
