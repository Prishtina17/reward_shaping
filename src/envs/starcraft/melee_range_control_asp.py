...
class Starcraft2EnvRewardShaping(StarCraft2Env):
    """Action, signed-state, and potential shaping with component metrics."""
    ...
    def reset(self):
        ...
        self._pending_action_match = None
        result = super().reset()
        self.phi_prev = self._state_score()[0]
        return result
    ...
    def reward_battle(self) -> float:
        ...
        state_score, cache = self._state_score()
        state_bonus = self._rc_weight * state_score
        terminal = self._episode_steps >= self.episode_limit
        phi_curr = 0.0 if terminal else state_score
        ...
        total_raw = action_bonus + state_bonus + potential_bonus
    ...
    def _state_score(self):
        ...
        score, cache = compute_ring_bonus_from_state(
            self,
            1.0,
            self._rc_melee_only,
            center,
