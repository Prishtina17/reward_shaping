...
> ⚠️ **HISTORICAL SNAPSHOT (2026-08-02).** Этот каталог сохраняет старую
> дипломную сдачу и не является source of truth: внутри остаются прежние
> `move_amount=3`, ratio-based clipping, старые shaping-классы и one-seed логи.
> Не запускайте код отсюда и не используйте его для описания финального
> протокола. Актуальная реализация и документация находятся в корне репозитория:
> [`../README.md`](../README.md) и [`../TODO.md`](../TODO.md).
